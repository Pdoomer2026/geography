...
export default function App() {
  ...
  const [folderPickerNeeded, setFolderPickerNeeded] = useState(false)
  ...
  useEffect(() => {
    initFileStore().then(async (ready) => {
      if (ready) {
        ...
        useGeoStore.getState().setFileStoreReady()
      } else if (!isElectron()) {
        // ブラウザかつ未許可 → フォルダ選択ボタンを表示
        setFolderPickerNeeded(true)
      }
    })
  }, [])
  ...
  return (
    <>
      ...
      {/* ブラウザ環境で fileStore 未許可の場合: フォルダ選択ボタン */}
      {folderPickerNeeded && (
        <div style={{
          ...
          <button
            onClick={async () => {
              ...
              if (ok) {
                await migrateFromLocalStorage()
                useGeoStore.getState().setFileStoreReady()
                setFolderPickerNeeded(false)
              }
            }}
            style={{
              padding: '6px 16px', borderRadius: 6,
              border: '1px solid #5a5aff', background: '#0d0d2a',
              color: '#aaaaff', fontSize: 11, fontFamily: 'monospace', cursor: 'pointer',
            }}
          >
            📂 GeoGraphy フォルダを選択
          </button>
