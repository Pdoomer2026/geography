/**
 * Macro8MidiWindow — MIDI Learn 対応版 MacroWindow
 *
 * Macro8Window をベースに以下を追加：
 *   - 右クリックコンテキストメニュー（MIDI Learn / Clear MIDI CC / Edit）
 *   - Learn 待機中の点滅アニメーション
 *   - CC バッジ表示（例: CC48）
 *   - engine.startMidiLearn() / getMidiLearnTarget() / getLearnedCC() との接続
 *
 * spec: docs/spec/midi-learn.spec.md
 * Day75: 動作確認用・確認後に Macro8Window と統合予定
 */
...
function ContextMenu({ x, y, knobId: _knobId, hasCC, onLearn, onClear, onEdit, onClose }: ContextMenuProps) {
  useEffect(() => {
...
function KnobCell({
  config, value, assignValues, learnedCC, isLearning,
  onEdit, onLearn, onClearCC, onDrop, onKnobChange,
}: KnobCellProps) {
  ...
  // Learn 中の点滅スタイル
  const learnBorderStyle = isLearning
    ? { animation: 'pulse-border 0.8s ease-in-out infinite', borderColor: '#ff7878' }
    : {}
    ...
    <>
      <div
        ...
        style={{
          width: KNOB_SIZE + 16,
          cursor: 'ns-resize',
          borderColor: isDragOver ? '#7878ff' : isLearning ? '#ff7878' : undefined,
      ...
      >
        <svg width={KNOB_SIZE} height={KNOB_SIZE} viewBox={`0 0 ${KNOB_SIZE} ${KNOB_SIZE}`}>
          {rings.map(({ r, assignIndex }) => {
            ...
            const ringValue = assign
              ? (assignValues[assignIndex] ?? 0)
              : (assignIndex === 0 ? localValue : 0)
              ...
              <g key={assignIndex}>
                ...
                {(hasAssign || assignIndex === 0) && ringValue > 0.001 && (
                  <path
                    ...
                    fill="none"
                    stroke={isLearning ? '#ff7878' : strokeColor}
                    strokeWidth={assignIndex === 0 ? 3.5 : 2.5}
                    strokeLinecap="round"
                  />
                )}
          ...
          {/* MIDI Learn 済み CC インジケーター */}
          {hasMidi && (
            <circle cx={KNOB_SIZE - 8} cy={8} r={4} fill={isLearning ? '#ff4444' : '#ff7878'} />
          )}
          {isDragOver && (
        ...
        {/* Learn 中 or 値 */}
        {isLearning ? (
          <span className="text-[9px] tabular-nums" style={{ color: '#ff7878' }}>
            Move a controller...
          </span>
        ) : (
        ...
        {/* CC バッジ */}
        {hasMidi && !isLearning && (
          <span className="text-[8px] px-1 rounded" style={{ color: '#ff9999', background: '#2a0a0a' }}>
            CC{learnedCC}
          </span>
        )}

        {/* アサイン一覧 */}
        {isAssigned && !isLearning && (
      ...
      {contextMenu && (
        <ContextMenu
          ...
          knobId={config.id}
          hasCC={hasMidi}
          onLearn={() => onLearn(config.id)}
          onClear={() => onClearCC(config.id)}
  ...
  return (
    ...
    >
      ...
      >
        ...
        <label className="block mb-3">
          <span className="text-[#6666aa] text-[9px] tracking-wider block mb-1">MIN</span>
        ...
        <label className="block mb-4">
          <span className="text-[#6666aa] text-[9px] tracking-wider block mb-1">MAX</span>
          <div className="flex items-center gap-2">
...
// ============================================================
// Macro8MidiWindow — メインコンポーネント
// ============================================================

export function Macro8MidiWindow() {
  ...
  const { pos, handleMouseDown } = useDraggable({ x: window.innerWidth / 2 - 440, y: 100 })
  ...
  const handleLearn = useCallback((id: string) => {
    const knob = engine.getMacroKnobs().find((k) => k.id === id)
    const label = knob?.name || id.replace('macro-', 'Macro #')
    engine.startMidiLearn({ id, type: 'macro', label })
  }, [])

  const handleClearCC = useCallback((id: string) => {
    engine.clearLearnedCC(id)
  }, [])
    ...
    <>
      <style>{`
        @keyframes pulse-border {
          0%, 100% { border-color: #ff7878; opacity: 1; }
          50% { border-color: #ff2222; opacity: 0.6; }
        }
      `}</style>
      ...
      >
        ...
        >
          <span>MACRO 8</span>
          <span className="text-[#3a3a5e]">MIDI LEARN</span>
          {learnTarget && (
            <span style={{ color: '#ff7878' }} className="animate-pulse">
              ● LEARNING {learnTarget.id.toUpperCase()}
            </span>
          )}
          <span className="ml-auto text-[#3a3a5e]">
            {knobs.filter(k => engine.getLearnedCC(k.id) >= 0).length} CC ASSIGNED
          </span>
        </div>
        ...
        >
          {knobs.map((knob, index) => (
            <KnobCell
              ...
              learnedCC={learnedCCs[index] ?? -1}
              isLearning={learnTarget?.id === knob.id}
              onEdit={handleEdit}
              onLearn={handleLearn}
              onClearCC={handleClearCC}
              onDrop={handleDrop}
        ...
        {learnTarget && (
          <div className="mt-2 text-center">
            <button
              onClick={() => engine.stopMidiLearn()}
              className="text-[9px] px-3 py-1 rounded border transition-colors"
              style={{ borderColor: '#3a1a1a', color: '#ff7878', background: '#1a0a0a' }}
            >
              Cancel Learn
            </button>
          </div>
