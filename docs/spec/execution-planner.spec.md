...
### 3レポ分割のタイミング（Day72 方針確定）

3レポ分割（UI / Application / Engine）は**製品版リリース直前**に行う。
SDK と本体をブラックボックス化して販売する段階まで、モノレポ1本で開発を続ける。

理由：
- 今は「作る」フェーズ → モノレポの方が開発速度が高い
- 分割コストが見合うのは「売る・公開する」直前のみ
- Engine を純粋計算に閉じ込める意識（`generateMesh(params) → mesh`）は
  今から保つことで、分割時のリファクタリングコストを最小化できる
