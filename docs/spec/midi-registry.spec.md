...
syncValues(registry, getEngineValue): MIDIRegistry       // 200ms ポーリングで value を同期
```
...
#### Phase B の未解決事項（Day57 全て解決）

1. **値域変換の責務** → Window が「表示用の化粧」として担当。Registry は 0.0〜1.0 の比率を運ぶだけ。
2. **cc-mapping.md の整備** → 未定義 param は CC1000〜 自動払い出しで対応済み。
3. **多重レイヤー問題** → bindings フェーズ（MacroKnob D&D）と一緒に設計する。
...
App.tsx（Orchestrator）
  ...
  → value: plugin.params[p.id].value で初期値を取得
  → RegisteredParameterWithCC を組み立てて registerParams() に渡す
  → 200ms ポーリングで syncValues() を呼び辺み、Registry の value を更新
...
WindowPlugin
  → Registry を filter するだけで params を取得できる（全種共通パターン）
  → 特定の Geometry を知らない
  → params.value が常に engine の現在値と同期されている（逆流）
  → 表示の min/max 変換は Window 側が「表示用の化粧」として担当
```
...
| `src/core/midiRegistry.ts` | 純粋関数（registerParams / clearParams / syncValues） |
| `src/ui/App.tsx` | Registry の React state・Orchestrator・200ms ポーリング |
